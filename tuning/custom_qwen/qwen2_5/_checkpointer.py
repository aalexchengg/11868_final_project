...
from tuning.custom_qwen.qwen2_5 import convert_weights
...
class CustomFullModelHFCheckpointer(_CheckpointerInterface):
    """
    ...
    def load_checkpoint(self) -> Dict[str, Any]:
        ...
        if self._model_type in (ModelType.PHI3_MINI, ModelType.PHI4):
            ...
            from tuning.custom_qwen.qwen2_5._convert_weights import (
                custom_qwen2_hf_to_tune,
            )

            converted_state_dict[training.MODEL_KEY] = custom_qwen2_hf_to_tune(
                merged_state_dict,
                num_heads=self._config["num_attention_heads"],
                num_kv_heads=self._config["num_key_value_heads"],
                dim=self._config["hidden_size"],
    ...
    def save_checkpoint(
        ...
        epoch: int,
        allowed_new_layer_types: List[str] = [],
        ...
        Args:
            ...
            allowed_new_layer_types (List[str]): List of new layer types (suffix) to be saved. Default is empty list.
        ...
        if not adapter_only:
            if self._model_type in (ModelType.PHI3_MINI, ModelType.PHI4):
                ...
                from tuning.custom_qwen.qwen2_5._convert_weights import (
                    custom_qwen2_tune_to_hf,
                )

                state_dict[training.MODEL_KEY] = custom_qwen2_tune_to_hf(
                    state_dict[training.MODEL_KEY],
                    num_heads=self._config["num_attention_heads"],
                    num_kv_heads=self._config["num_key_value_heads"],
                    dim=self._config["hidden_size"],
            ...
            # get last cpt_idx
            last_cpt_idx = len(self._checkpoint_paths)

            for key, weight in state_dict[training.MODEL_KEY].items():
                # print(self._weight_map.keys())
                if key in self._weight_map:
                    cpt_idx = self._weight_map[key]
                elif any(
                    key.endswith(layer_type) for layer_type in allowed_new_layer_types
                ):
                    cpt_idx = last_cpt_idx
                else:
                    raise ValueError(
                        f"Key {key} not found in weight_map and not in allowed_new_layer_types"
                    )
