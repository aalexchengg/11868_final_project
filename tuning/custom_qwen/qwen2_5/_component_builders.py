...
from tuning.custom_qwen.peft import (
...
from tuning.custom_qwen.propulsion import PropulsionLinear
...
def propulsion_qwen2(
    ...
    # Propulsion args
    prop_attn_modules: List[LORA_ATTN_MODULES] = None,
    ...
    Args:
        ...
        prop_degree (int): degree of the propulsion function
        ...
        apply_prop_to_mlp (bool): whether to apply Propulsion to the MLP in each transformer layer.
            Default: False
        apply_prop_to_output (bool): whether to apply Propulsion to the model's final output projection.
            Default: False
    Returns:
        TransformerDecoder: Instantiation of Qwen2 model.

    Raises:
        ValueError: if ``apply_prop_to_output`` and ``tie_word_embeddings``.
    """

    if not prop_attn_modules:
        raise ValueError(
            "Must pass one or more of {LORA_ATTN_MODULES} as prop_attn_modules"
        )
    ...
    for _ in range(num_layers):
        self_attn = MultiHeadAttention(
            ...
            q_proj=(
                PropulsionLinear(
                    embed_dim, num_heads * head_dim, bias=True, degree=prop_degree
                )
                if "q_proj" in prop_attn_modules
                else nn.Linear(embed_dim, num_heads * head_dim, bias=True)
            ),
            k_proj=(
                PropulsionLinear(
                    embed_dim, num_kv_heads * head_dim, bias=True, degree=prop_degree
                )
                if "k_proj" in prop_attn_modules
            ...
            v_proj=(
                ...
                if "v_proj" in prop_attn_modules
                else nn.Linear(embed_dim, num_kv_heads * head_dim, bias=True)
            ),
            output_proj=(
                PropulsionLinear(embed_dim, embed_dim, bias=False, degree=prop_degree)
                if "output_proj" in prop_attn_modules
        ...
        if apply_prop_to_mlp:
            mlp = qwen2_propulsion_mlp(
                dim=embed_dim, hidden_dim=intermediate_dim, degree=prop_degree
            )
        else:
    ...
    if tie_word_embeddings:
        if apply_prop_to_output:
            raise ValueError(
                "apply_prop_to_output is incompatible with tie_word_embeddings"
            )
        output_proj = TiedLinear(tok_embeddings)
    else:
        if apply_prop_to_output:
            output_proj = PropulsionLinear(
                embed_dim, vocab_size, bias=False, degree=prop_degree
            )
        else:
...
def qwen2_propulsion_mlp(dim: int, hidden_dim: int, degree: int) -> FeedForward:
    """
    Build the MLP layer associated with the Qwen2 model.
    """
    gate_proj = PropulsionLinear(dim, hidden_dim, bias=False, degree=degree)
    down_proj = PropulsionLinear(hidden_dim, dim, bias=False, degree=degree)
    up_proj = PropulsionLinear(dim, hidden_dim, bias=False, degree=degree)
    return FeedForward(gate_proj=gate_proj, down_proj=down_proj, up_proj=up_proj)


def lora_propulsion_qwen2(
    ...
    *,
    ...
    layers = nn.ModuleList()
    for _ in range(num_layers):
        self_attn = lora_propulsion_qwen2_self_attention(
    ...
    if tie_word_embeddings:
        ...
        if apply_prop_to_output:
            adapter_cls = PropulsionDoRALinear if use_dora else PropulsionLoRALinear
            output_proj = (
                adapter_cls(
                    embed_dim,
                    vocab_size,
...
def lora_propulsion_qwen2_self_attention(
    ...
    *,
    ...
    Args:
        ...
        prop_attn_modules (List[LORA_ATTN_MODULES]): list of which linear layers
            Propulsion should be applied to. Options are ``{"q_proj", "k_proj", "v_proj",
            "output_proj"}``.
        prop_degree (int): degree of the propulsion function
    Returns:
    ...
    if not prop_attn_modules:
        raise ValueError(
            f"Must pass one or more of {LORA_ATTN_MODULES} as prop_attn_modules"
        )
    ...
    if "output_proj" in prop_attn_modules:
        ...
        output_proj = (
            ...
            else PropulsionLinear(embed_dim, embed_dim, bias=False, degree=prop_degree)
        )
    else:
        adapter_cls = DoRALinear if use_dora else LoRALinear
        output_proj = (
